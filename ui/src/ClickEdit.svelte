...
<div class="click-edit">
    <div class="editing" class:hidden={!isEditing}>
        ...
        {#if confirmButtons}
            ...
            <sl-button on:mousedown={doCancel}>
                <SvgIcon icon=faCancel size=12px/>
            </sl-button>
        {/if}
    </div>
    ...
    >
        ...
        <span  on:click={(e)=>{
            e.stopPropagation();
            isEditing = true;
        }}> {text} <SvgIcon size=12px icon=faEdit style="opacity: .3; height: .875rem; margin-left: 3px; position: relative; top: -.15rem"/></span >
        {#if handleDelete} 
            <span  on:click={(e)=>{
                e.stopPropagation();
                handleDelete()
            }}><SvgIcon icon=faTrash size=12px style="opacity: .3; height: .875rem; margin-left: 3px; position: relative; top: -.15rem"/></span >
        {/if}        
    </div>
</div>
