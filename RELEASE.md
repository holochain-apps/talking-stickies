# Releasing TalkingStickies

TalkingStickies is distributed as a `.webhapp` referenced by the
...
and 0.6 and 0.7 agents form disjoint networks. Boards created on the 0.6 line
(TalkingStickies 0.15.x) do not carry over, and a group has to move across together — a
0.7 agent cannot see 0.6 peers at all. To carry board contents over, use the
board menu's **Export** item (or **Export All Boards** in the About dialog) on
the 0.6 install, then **Import Boards** in the About dialog on the 0.7 one.
...
over every zome named in `dnas/talking-stickies/workdir/dna.yaml` (`wasm-opt -Oz
...
The script verifies `workdir/talking-stickies.happ` against `.happ-sha256` and
...
3. The [`release-webhapp`](.github/workflows/release-webhapp.yaml) workflow then:
   - downloads the frozen happ from `happ-v<dnaVersion>` and checks its sha256
     against `.happ-sha256`,
   - builds the UI and packs `talking-stickies.webhapp` (no `--recursive`, so the happ
   ...
   - publishes a **prerelease** GitHub release with `talking-stickies.webhapp` attached.
...
Add a new `versions[]` entry for `talking-stickies` in the 0.16 curation list with the
new `version`, the release's `talking-stickies.webhapp` `url`, and these hashes. Because
