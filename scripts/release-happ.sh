...
#   1. `workdir/talking-stickies.happ` from a local `npm run build:happ:release` — used when
...
#      packing); plain `build:happ` skips that and produces DIFFERENT bytes, which
...
if [ $# -ge 1 ]; then
  ...
  happ="$tmp/wa/talking-stickies.happ"
else
  happ="workdir/talking-stickies.happ"
  if [ ! -f "$happ" ]; then
    echo "ERROR: $happ not found. Run 'npm run build:happ:release' first" >&2
    echo "(the canonical build; plain 'build:happ' produces a different happ)," >&2
...
notes="Frozen canonical talking-stickies.happ reused by every webhapp release to keep all installs on the same network. DNA sha256: $EXPECTED_SHA. Do NOT rebuild."
...
echo "Done. release-webhapp.yaml will download talking-stickies.happ from $HAPP_TAG."
